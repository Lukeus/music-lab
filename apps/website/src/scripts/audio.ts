...
function showToast(msg: string) {
  ...
  const host = mini ? mini.querySelector('#toast-host') : document.getElementById('toast-host');
  if (!host) return;
...
export function bindMiniPlayer() {
    ...
    play.addEventListener('click', () => {
      ...
      if (currentAudio.paused) {
        ...
        if (currentButton) {
          ...
          const els = currentButton.closest('.project-card, .experiment-card');
          const canvas = els ? els.querySelector('canvas.waveform-canvas') as HTMLCanvasElement : null;
          stopWaveform();
  ...
  if (progress && !(progress as any).__bound) {
    ...
    progress.addEventListener('click', (e: MouseEvent) => seekFromClientX(e.clientX));
...
function syncMiniTo(btn: HTMLElement) {
  ...
  title = heading && typeof heading.textContent === 'string' ? heading.textContent.trim() : null;
  }
  ...
  if (mini.play) {
    mini.play.innerHTML = currentAudio && !currentAudio.paused ? iconPauseSVG() : iconPlaySVG();
  }
}
...
export function initSharedAudio() {
  ...
  if (root) root.classList.add('show');
    ...
    currentAudio.addEventListener("timeupdate", () => {
      const { timeCurrent, timeDuration, progressFill } = getMiniElements();
...
export function bindWaveform() {
  ...
  function resizeWaveforms() {
    document.querySelectorAll<HTMLCanvasElement>('.waveform-canvas').forEach(cv => {
