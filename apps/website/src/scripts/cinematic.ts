...
class CinematicParticleSystem {
  ...
  private connectAudioAnalysis(): void {
      ...
      if (audioElements.length > 0) {
        audioElements.forEach(audio => {
          ...
          if (!(audio as any).__cinematicBound) {
            ...
            audio.addEventListener('play', () => this.setAudioPlaying(true));
            audio.addEventListener('pause', () => this.setAudioPlaying(false));
            audio.addEventListener('ended', () => this.setAudioPlaying(false));
            
            // Connect to audio analysis if available
            audio.addEventListener('timeupdate', () => this.onAudioTimeUpdate(audio));
          }
        });
      }
    ...
    document.addEventListener('audioPlay', () => this.setAudioPlaying(true));
    document.addEventListener('audioPause', () => this.setAudioPlaying(false));
    document.addEventListener('audioStop', () => this.setAudioPlaying(false));
  }
  ...
  private calculateFakeIntensity(audio: HTMLAudioElement): number {
    ...
    return Math.max(0, Math.min(1, base + variation * (slow + medium + fast)));
  }
