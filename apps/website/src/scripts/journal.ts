...
export function bindJournalFeatures() {
  ...
  function entryMatches(entry: HTMLElement, q: string, tag: string): boolean {
    const title = entry.querySelector('.journal-title')?.textContent?.toLowerCase() || '';
    const snippet = entry.querySelector('.journal-snippet')?.textContent?.toLowerCase() || '';
  ...
  const q = (search && 'value' in search ? (search as HTMLInputElement).value : '').toLowerCase();
    const activeTagBtn = tagButtons.find(b => b.classList.contains('is-active'));
    const tag = activeTagBtn ? activeTagBtn.getAttribute('data-tag') || 'all' : 'all';
    if (!entriesWrap) return;
    const items = Array.from(entriesWrap.querySelectorAll('.journal-card')) as HTMLElement[];
...
function disableStickyJournalHeaders() {
  ...
  document.querySelectorAll('.journal-head').forEach(h => {
    ...
    if (cs.position === 'sticky' || (h as HTMLElement).style.position === 'sticky') {
