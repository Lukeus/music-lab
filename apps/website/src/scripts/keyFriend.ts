...
export function generateMajorKey(tonic: Note): Key {
    ...
    const MAJOR_CHORD_PATTERNS: Array<{
        numeral: ChordNumeral;
        type: ChordType;
        intervals: number[];
    }> = [
    ...
    const chords: Chord[] = MAJOR_CHORD_PATTERNS.map(pattern => {
        const chordNotes = pattern.intervals.map(
...
export function generateMinorKey(tonic: Note): Key {
    ...
    const MINOR_CHORD_PATTERNS: Array<{
        numeral: ChordNumeral;
        type: ChordType;
        intervals: number[];
    }> = [
...
export class KeyFriend {
    ...
    public on(
        event: 'progressionSelect',
        callback: (progression: string) => void
    ): void;
    ...
    public setKey(tonic: Note, mode: 'major' | 'minor' = 'major'): void {
        const newKey =
            mode === 'major'
                ? generateMajorKey(tonic)
                : generateMinorKey(tonic);
    ...
    public toggleChord(chord: Chord): void {
        const existingIndex = this.state.selectedChords.findIndex(
            c => c.root === chord.root && c.type === chord.type
        );
    ...
    public applyProgression(
        progressionName: keyof typeof COMMON_PROGRESSIONS
    ): void {
        const progression = COMMON_PROGRESSIONS[progressionName];
        const progressionChords = progression
            .map(numeral =>
                this.state.currentKey.chords.find(
                    chord => chord.numeral === numeral
                )
            )
    ...
    private renderChords(): string {
        return this.state.currentKey.chords
            .map((chord, index) => {
                const isSelected = this.state.selectedChords.some(
                    c => c.root === chord.root && c.type === chord.type
                );
    ...
    private updateKeyDisplay(): void {
        const keyNameEl = this.container.querySelector(
            '.current-key-name'
        ) as HTMLElement;
        const scaleNotesEl = this.container.querySelector(
            '.scale-notes'
        ) as HTMLElement;
        ...
        if (scaleNotesEl) {
            ...
            setTimeout(() => {
                scaleNotesEl.innerHTML = this.state.currentKey.scale
                    .map(
                        note => `
          <span class="scale-note">${note}</span>
        `
    ...
    private updateChordsDisplay(): void {
        const chordsGrid = this.container.querySelector(
            '#chords-grid'
        ) as HTMLElement;
        if (chordsGrid) {
        ...
        const toggleNotesBtn = this.container.querySelector(
            '.toggle-notes-btn'
        ) as HTMLElement;
        if (toggleNotesBtn) {
    ...
    private updateProgressionsDisplay(): void {
        const progressionBtns =
            this.container.querySelectorAll('.progression-btn');
    ...
    private attachEventListeners(): void {
        ...
        const tonicSelect = this.container.querySelector(
            '#key-tonic'
        ...
        tonicSelect?.addEventListener('change', e => {
            ...
            const mode =
                (this.container
                    ...
                    ?.getAttribute('data-mode') as 'major' | 'minor') ||
                'major';
        ...
        this.container.addEventListener('click', e => {
            ...
            if (target.classList.contains('mode-btn')) {
                ...
                const mode = target.getAttribute('data-mode') as
                    | 'major'
                    | 'minor';
                const tonic = (
            ...
            if (target.closest('.chord-card')) {
                ...
                const chordIndex = parseInt(
                    chordCard.getAttribute('data-chord-index') || '0'
                );
            ...
            if (target.closest('.progression-btn')) {
                const progressionBtn = target.closest(
                    '.progression-btn'
                ) as HTMLElement;
                const progressionName = progressionBtn.getAttribute(
                    'data-progression'
                ) as keyof typeof COMMON_PROGRESSIONS;
                if (progressionName) {
