...
import { createScaleExplorer, Scale, Note, ScaleCategory } from '@lukeus/music-tools';
...
function setupScaleExplorerCallbacks(scaleExplorer: any) {
  ...
  scaleExplorer.on('visualModeChange', (mode: 'keyboard' | 'fretboard' | 'circle') => {
    console.log('👀 Visual mode changed to:', mode);
    
    // Add transition effects specific to the website
    const visualContainer = document.querySelector('.scale-visual-display');
    if (visualContainer) {
